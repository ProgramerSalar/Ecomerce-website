...
export const newOrder = TryCatch(async (req, res, next) => {
    ...
    if (!user)
        return next(new ErrorHandler("Please Enter userId", 404));
    if (!subtotal)
        return next(new ErrorHandler("Please Enter subtotal", 404));
    if (!tax)
        return next(new ErrorHandler("Please Enter Tax", 404));
    if (!shippingCharges)
        return next(new ErrorHandler("Please Enter shippingCharges", 404));
    if (!discount)
        return next(new ErrorHandler("Please Enter discount", 404));
    if (!total)
        return next(new ErrorHandler("Please Enter total", 404));
